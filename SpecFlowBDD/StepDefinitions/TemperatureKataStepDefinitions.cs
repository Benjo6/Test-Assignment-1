...
namespace SpecFlowBDD.StepDefinitions
{
    ...
    public class TemperatureConverterStepDefinitions
    {
        ...
        public void ThenTheResultOfConversionToCelsiusWillBeDegrees(double celsius)
        {
            if (_kelvin != -500)
            {
                Assert.AreEqual(celsius, (double)_scenarioContext["KelvinToCelsius"]);
            }
            else
            {
                Assert.AreEqual(celsius, (double)_scenarioContext["FahrenheitToCelsius"]);
        ...
        public void ThenTheResultOfConversionToKelvinWillBeDegrees(double kelvin)
        {
            if (_fahrenheit != -500)
            {
                Assert.AreEqual(kelvin, (double)_scenarioContext["FahrenheitToKelvin"]);
            }
            else
            {
                Assert.AreEqual(kelvin, (double)_scenarioContext["CelsiusToKelvin"]);
        ...
        public void ThenTheResultOfConversionToFahrenheitWillBeDegrees(double fahrenheit)
        {
            if (_celsius != -500)
            {
                Assert.AreEqual(fahrenheit, (double)_scenarioContext["CelsiusToFahrenheit"]);

            }
            else
            {
                Assert.AreEqual(fahrenheit, (double)_scenarioContext["KelvinToFahrenheit"]);
            }
        }
